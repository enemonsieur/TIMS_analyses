...
OUTPUT_DIRECTORY = Path(r"C:\Users\njeuk\OneDrive\Documents\Charite Berlin\TIMS\evidence\skript_probe_pass4_mne_outputs")
OUTPUT_DIRECTORY.mkdir(parents=True, exist_ok=True)

# This probe stays on one channel because the question is only whether the old
# stim recording differs visibly from baseline at Cz in time and PSD.
CHANNEL_NAME = "Cz"

# The first 20 s are enough for a quick readable comparison without forcing the
# reader through a long continuous trace.
WINDOW_START_S = 0.0
WINDOW_DURATION_S = 20.0

# A broad overview band is enough here because this is a readable probe, not a
# narrowband mechanistic analysis.
...
# ===== Prepare Cz =============================================================
# Keep the filtering inline and MNE-native because the operation is simple and
# easier to trust here than a helper that hides the exact filter choice.
...
# ===== PSD ====================================================================
# Use fixed-length 5 s epochs so baseline and stim stay directly comparable in
# a simple Welch PSD summary.
...
if len(baseline_epochs) < 1 or len(stim_epochs) < 1:
    raise RuntimeError("Need at least one epoch in each recording for PSD.")
...
summary_lines = [
    "Cz baseline vs stim probe pass 4",
