...
**Core Problem:** Causal sosfilt bandpass (12.5–13.5 Hz, Q=13) causes forward ringing for 2+ s post-pulse. ITPC/PLV computed on filtered signal may measure artifact phase locking, not true signal recovery. Need to determine optimal filtering strategy and ITPC computation method.

**Investigation Plan:**

1. **Pulse timing alignment QC** (prerequisite): Quantify pulse jitter via cross-correlation of STIM epochs. Determine if alignment error exists; improve pulse detection function (threshold, rise-time parameters) if needed. Report jitter magnitude and decision on usability.
...
4. **Filtering method optimization** (ongoing learning): Audit where filtering occurs in `explore_exp08_art_filtering.py` pipeline (before/after component selection?), test alternative filter designs (bandwidth trade-offs, notch effectiveness), and validate SNR/ITPC stability across intensities. Documented step-by-step as methodology is tested.

5. **Replace ITPC visualization with PLV** (if applicable): Current 6-panel ITPC timecourse may be noisy in high-artifact regime. Test PLV (phase locking value) as alternative summary metric; decide which to prioritize based on stability across intensities.
