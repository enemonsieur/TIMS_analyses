/*
    ...
    Output is updated once every N samples.
...
module cross_correlation_two_signals_sparse(clk, rst, sample_ready, sample1, sample2, out, output_ready);
    ...
    // Number of samples summed over so far; resets every N samples
    reg [N_NBITS-1:0] sum_index = 0;
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (sample_ready && !sample_ready_z) begin // new sample
                ...
                // pull new data in multiplier inputs
                sample1_reg <= sample1;
                sample2_reg <= sample2;

            end
            else if (working) begin
                // Step 0: multiply and store output in a register
                if (pipeline_index == 0)
                    sample1_times_sample2_reg <= sample1_reg * sample2_reg;

                // Step 1: accumulate multiplication results
                if (pipeline_index == 1)
                    sum_scaled <= sum_scaled + sample1_times_sample2_reg;

                // Step 2: output result
                if (pipeline_index == 2) begin
                    working <= 0;

                    if (sum_index == N-1) begin // N samples done
                        sum_index <= 0;
                        sum_scaled <= 0;
                        output_ready <= 1;
